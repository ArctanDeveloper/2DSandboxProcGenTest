...
void Command::AddParameter(ParameterType param, bool required) {
    parameters.emplace_back(param, required);
}

bool Command::IsValid(std::string initializer, const std::vector<Parameter>& parameters) {
    if (name != initializer) {
        return false;
    }

    for (int i = 0; i < this->parameters.size(); i++) {
        if (parameters[i].index() != std::get<0>(this->parameters[i]) && std::get<1>(this->parameters[i])) {
            return false;
        }
    }
