...
TEST(VectorTest, UnitTest) {
    ...
    EXPECT_EQ(Vec2i::UNIT_X.Normalize().x, Vec2i::UNIT_XY.Normalize().x) << "Expected UNIT_X x to be greater than normalized UNIT_XY x.";
    EXPECT_EQ(Vec2i::UNIT_Y.Normalize().y, Vec2i::UNIT_XY.Normalize().y) << "Expected UNIT_Y y to be greater than normalized UNIT_XY y.";
